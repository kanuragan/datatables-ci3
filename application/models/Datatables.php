  ...
  */
  class Datatables extends CI_Model
  {
    ...
    */
    ...
    {
      $this->order = array($column,$sort);
    }
    public function order_string()
    {
      if(isset($_POST['order'])) 
        {
            return ($this->order_column[$_POST['order']['0']['column']] ." ". $_POST['order']['0']['dir']);
        } 
        else if(isset($this->order))
        {
            $order = $this->order;
            return (key($order) ." ". $order[key($order)]);
        }
    }
    public function order_column($columns, $backtick_protect = TRUE)
    {
      foreach($this->explode(',', $columns) as $val)
      {
        ...
        $this->columns[] =  $column;
        $this->order_column[$column] =  trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$1', $val));
      }
    ...
    */
    ...
    public function where_not_in($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
      ...
      $this->db->where_not_in($key_condition, $val, $backtick_protect);
      return $this;
    }
    public function order_by($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
      ...
      $this->db->order_by($key_condition, $val, $backtick_protect);
      return $this;
    }
      ...
      if($sSearch != '')
        for($i = 0; $i < count($mColArray); $i++)
          if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns))
            if($this->check_cType())
              $sWhere .= "LOWER(CONCAT(".$mColArray[$i]['data']."))" . " LIKE '%" . strtolower($sSearch) . "%' OR ";
            else
              $sWhere .= "LOWER(CONCAT(".$this->columns[$i]."))" . " LIKE '%" . strtolower($sSearch) . "%' OR ";
    ...
    */
    ...
    {
      $aaData     = $this->get_display_result()->result_array();

      $this->query = explode('LIMIT', $this->db->last_query())[0];
    ...
    */
    private function get_total_results($filtering = FALSE)
    {
      ...
      $subquery = $this->db->get_compiled_select($this->table);
